...
function showBadge() {
  // Only show when chat panel is closed
  if (!chatOpen) chatBadge?.classList.add("visible");
}
function clearBadge() {
...
function connectSocket() {
  socket = io("https://peekr-production-c72b.up.railway.app", {
...
function wireChannel(ch) {
  ...
  ch.onmessage = ({ data }) => {
    ...
    showBadge(); // notify if chat is closed
  };
}
